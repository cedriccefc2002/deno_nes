...
export class CPU {
    ...
    F_CARRY: number = 0;
    F_DECIMAL: number = 0;
    F_INTERRUPT: number = 0;
    F_INTERRUPT_NEW: number = 0;
    F_OVERFLOW: number = 0;
    F_SIGN: number = 0;
    F_ZERO: number = 0;
    F_NOTUSED: number = 0;
    F_NOTUSED_NEW: number = 0;
    F_BRK: number = 0;
    F_BRK_NEW: number = 0;
    opdata: number[] = [];
    cyclesToHalt: number = 0;
    crash: boolean = false;
    irqRequested: boolean = false;
    irqType: IRQ = IRQ.NORMAL;
    constructor(private nes: any) {
        this.reset();
    }
    public reset() {
        // Main memory
        this.mem = new Array<number>(0x10000);
        ...
        this.irqType = IRQ.NORMAL;
    }
    public load(addr: number) {
        if (addr < 0x2000) {
    ...
    public pageCrossed(addr1: number, addr2: number) {
        return (addr1 & 0xff00) !== (addr2 & 0xff00);
    }

    public haltCycles(cycles: number) {
        this.cyclesToHalt += cycles;
    }

    public doNonMaskableInterrupt(status: number) {
